    ...
    <div className={"mt-8 rounded-2xl border px-5 py-4 " + panelCls}>
      <p
        className={
          ...
          (highlighted ? "text-white/70" : "text-brand-violet-strong")
        }
      >
        Payback estimator
      </p>
      ...
      <div
        className={"mt-4 border-t pt-3 " + (highlighted ? "border-white/15" : "border-border/70")}
      >
        <p className="text-sm">
        ...
        <p className="mt-1.5 text-sm leading-snug">
          ...
          {months !== null && weeks !== null && weeks >= 4 ? (
            <span className={subCls}>
              {" "}
              (~{months < 1 ? "1 month" : `${Math.round(months)} months`})
            </span>
          ) : null}
        </p>
        <p
          className={
            "mt-2.5 text-[10.5px] leading-relaxed " +
            (highlighted ? "text-white/45" : "text-muted-foreground/80")
          }
        >
...
export function BusinessRevenueCalculator({
  ...
  return (
    <section
      aria-labelledby="revenue-calculator-title"
      className="mx-auto max-w-4xl px-6 py-20 md:px-10 md:py-24"
    >
      <div className="grid gap-10 border-y border-border py-10 md:grid-cols-[0.9fr_1.1fr] md:items-center">
        <div>
          ...
          <h2
            id="revenue-calculator-title"
            className="mt-3 text-3xl font-light text-brand-indigo md:text-4xl"
          >
            Explore your session revenue
          </h2>
          <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
            Adjust the sessions you expect to offer and your average price. This estimates gross
            session revenue, not profit.
          </p>
        ...
        <div className="rounded-lg border border-border bg-card p-6 shadow-soft">
          ...
          <div className="mt-6 grid gap-4 border-t border-border pt-5 sm:grid-cols-2">
            <div>
              <p className="text-xs text-muted-foreground">Estimated gross revenue / week</p>
              <p className="mt-1 text-2xl font-medium text-brand-indigo">
                {money(weeklyRevenueCents)}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Package price covered in</p>
              <p className="mt-1 text-2xl font-medium text-brand-indigo">
                About {Math.ceil(weeks)} weeks
              </p>
            </div>
          ...
          <p className="mt-5 text-xs leading-relaxed text-muted-foreground">
            Illustrative only. Operating costs, tax treatment, utilisation and actual demand affect
            take-home returns. This is not a guarantee of income.
          </p>
        </div>
      </div>
    </section>
